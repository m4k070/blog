...
let make = () => {
  ...
  switch url.path {
    | list{"articles", id} =>
      <div className="p-6">
        <h1 className="text-3xl font-semibold"> {React.string("Article ID: " ++ id)} </h1>
        <p> {React.string("This is a placeholder for the article with ID: " ++ id)} </p>
      </div>
    | s =>
    <div className="p-6">
      <h1 className="text-3xl font-semibold"> {"What is this about?"->React.string} </h1>
      <p>
        {List.map(s, React.string) |> List.toArray |> React.array}
